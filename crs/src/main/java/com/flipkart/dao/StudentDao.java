...
public class StudentDao implements StudentDaoInterface{
	...
	public float register(Student student, String courseID) throws CourseAlreadyOptedException, CourseNotAvailableException, CourseNotFoundException{
		...
		try {
			PreparedStatement psCheck = conn.prepareStatement(Queries.CHECK_STUDENT_COURSE);
			...
			PreparedStatement ps = conn.prepareStatement(Queries.GET_COURSE);
			...
			PreparedStatement ps1 = conn.prepareStatement(Queries.DECR_SEATS);
			...
			PreparedStatement ps2 = conn.prepareStatement(Queries.REGISTER_COURSE);
	...
	public List<Course> viewCoursesEnrolled(Student student) {
		...
		try {
			...
			PreparedStatement ps = conn.prepareStatement(Queries.GET_COURSE_ENROLLED);
	...
	public ReportCard getReport(Student student) {
		...
		try {
			...
			PreparedStatement ps = conn.prepareStatement(Queries.GET_GRADES);
	...
	public Billing getBillingInfo(Student student) throws BillingNotFoundException{
		// TODO Auto-generated method stub
		try {
			PreparedStatement ps = conn.prepareStatement(Queries.GET_BILLING_INFO);
            ps.setString(1, student.getID());
            ResultSet rs = ps.executeQuery(); 
            if(rs.next()) {
    ...
    public boolean updateBillingInfo(Billing billing) {
        try {
            PreparedStatement ps = conn.prepareStatement(Queries.UPDATE_BILLING_INFO);
