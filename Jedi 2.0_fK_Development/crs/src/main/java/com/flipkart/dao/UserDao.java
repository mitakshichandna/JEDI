...
public class UserDao implements UserDaoInterface{
	...
	public User getUser(String username) throws UserNotFoundException {
		...
		try{
			PreparedStatement ps = conn.prepareStatement(Queries.GET_USER_USERNAME);
	...
	User getStudent(String userID, String name, String role,
		...
		try{
			PreparedStatement ps = conn.prepareStatement(Queries.GET_STUDENT_USERID);
			...
			if (rs.next()) {
				...
				user = new Student(userID,name,contact,email,branch,rollNum,approved,password);
				if (!approved) {
	...
	User getProfessor(String userID, String name, String role,
		...
		try{
			PreparedStatement ps = conn.prepareStatement(Queries.GET_PROFESSOR_USERID);
			...
			if (rs.next()) {
				...
				//int rollNum=rs.getInt("rollNum");
				//boolean approved=rs.getBoolean("approved");
				user = (User) new Professor(userID,name,contact,email,department,qualification,password);
				return user;
			}else return null;
		}catch(SQLException e) {
			return null;
	...
	User getAdmin(String userID, String name, String role,
			String contact, String email, String password) {
		User user = new Admin(userID,name,contact,email,password);
		return user;
	}
	...
	public String registerStudent(String username, String name, String contact, String email, String password, String branch) throws UsernameAlreadyInUseException {
		...
		try {
			...
			PreparedStatement ps = conn.prepareStatement(Queries.VERIFY_USERNAME);
		...
		try {
			...
			ps = conn.prepareStatement(Queries.FETCH_ROLLNUMS);
			rs=ps.executeQuery();
			...
			ps = conn.prepareStatement(Queries.ADD_USER);
			ps.setString(1, userID);
			ps.setString(2, name);
			...
			ps.executeUpdate();ps = conn.prepareStatement(Queries.ADD_STUDENT);
			ps.setString(1, userID);
			...
			ps.executeUpdate();
			//
			PreparedStatement ps1 = conn.prepareStatement(Queries.FETCH_BILLINGS);
			...
			ps1 = conn.prepareStatement(Queries.ADD_BILLING);
	...
	public boolean updatePassword(String username, String oldPassword, String newPassword) throws UserNotFoundException {
		// TODO Auto-generated method stub
		try {
			PreparedStatement ps1 = conn.prepareStatement(Queries.GET_USER_USERNAME);
			...
			if (rs.next())
			{
			PreparedStatement ps = conn.prepareStatement(Queries.UPDATE_PASSWORD);
